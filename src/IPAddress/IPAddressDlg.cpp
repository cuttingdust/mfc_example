...
CIPAddressDlg::CIPAddressDlg(CWnd* pParent /*=nullptr*/) : CDialogEx(IDD_IPADDRESS_DIALOG, pParent), m_strPort(_T(""))
{
...
ON_WM_PAINT()
ON_WM_QUERYDRAGICON()
ON_BN_CLICKED(IDC_GET_IP, &CIPAddressDlg::OnBnClickedGetIp)
END_MESSAGE_MAP()


// CIPAddressDlg 消息处理程序
...
void CIPAddressDlg::OnBnClickedGetIp()
{
    // TODO: 在此添加控件通知处理程序代码
    BYTE    nFild[4]; /// 分别存放点分十进制的每一个字节
    CString sip;      /// IP地址的字符串形式
    UpdateData();
    if (m_IP.IsBlank())
    {
        AfxMessageBox(L"请填写合适的IP地址");
        return;
    }
